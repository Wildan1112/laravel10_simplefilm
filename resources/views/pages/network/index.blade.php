            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex mb-3">
                        <a href="{{ route('network.create') }}" class="btn btn-sm btn-primary btn-icon-text">
                            <i class="btn-icon-prepend" data-feather="plus-circle"></i>
                            Add Network
                        </a>
                        <a href="{{ route('import.network') }}" class="btn mx-1 btn-sm btn-info btn-icon-text">
                            <i class="btn-icon-prepend" data-feather="file-plus"></i>
                            Import from Excel
                        </a>
                        <form action="{{ route('export.network') }}" method="GET">
                            ...
                            <tbody>
                                @foreach ($networks as $network)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('network.edit', $network->slug) }}"
                                                    class="btn mx-1 btn-xs btn-warning btn-icon">
                                                    <i data-feather="edit"></i>
                                                </a>
                                                <form action="{{ route('network.destroy', $network->slug) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
